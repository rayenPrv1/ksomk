...
console.log(`\x1b[37mName: \x1b[36m${client.user.tag} \n\x1b[37mID: \x1b[32m${client.user.id} \n\x1b[37mServers: \x1b[33m${client.guilds.cache.size} \x1b[0m`)
client.user.setStatus("online");	client.user.setActivity('valerie Server')
});
...
app.get("/successss", (req, res) => {
  res.sendFile(process.cwd() + "/successss.html");
});
...
client.on("messageCreate", async message => {
  ...
  } else if(args[0] === prefix + "users") {
    let data = await db.find();
    message.reply({ content: ` **totel :** ${data ? data.length : 0}` });
